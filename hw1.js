...
const Product = class Product {
    ...
    constructor(propertiesByProduct){
        this.propertiesArray = propertiesByProduct.split(",");
        this.name = this.propertiesArray[0];
        this.price = parseFloat(this.propertiesArray[1]);
        this.availability = this.propertiesArray[2];
    }
    ...
    static printProducts = (products) => {
        const response = (product) => {
            return product.availability === "In Stock" ? "Yes" : "No";
        };
    
        products.forEach(element => {
            console.log(`Product: ${element.name}, Cost: $${element.price.toFixed(2)}, Availability: ${response(element)}`);
        });
    };
